...
data class UwbDevice (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): UwbDevice {
      ...
      val rssi = list[5].let { if (it is Int) it.toLong() else it as Long? }
      return UwbDevice(id, name, uwbData, deviceType, state, rssi)
    }
  }
  fun toList(): List<Any?> {
    return listOf<Any?>(
...
interface UwbHostApi {
  ...
  fun discoverDevices(deviceName: String, auHourlyDigest: String, callback: (Result<Unit>) -> Unit)
      ...
      run {
        ...
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            val auHourlyDigestArg = args[1] as String
            api.discoverDevices(deviceNameArg, auHourlyDigestArg) { result: Result<Unit> ->
              val error = result.exceptionOrNull()
              if (error != null) {
                reply.reply(wrapError(error))
              } else {
  ...
  fun onHostUwbSessionDisconnected(deviceArg: UwbDevice, callback: (Result<Unit>) -> Unit)
{
  ...
  fun onUwbRangingResult(dataArg: UwbData, callback: (Result<Unit>) -> Unit)
{
    val channelName = "dev.flutter.pigeon.uwb.UwbFlutterApi.onUwbRangingResult"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
  ...
  fun onUwbRangingError(errorArg: String, callback: (Result<Unit>) -> Unit)
{
    val channelName = "dev.flutter.pigeon.uwb.UwbFlutterApi.onUwbRangingError"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
