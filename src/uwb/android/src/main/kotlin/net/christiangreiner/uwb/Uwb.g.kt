...
data class UwbDevice (
  ...
  companion object {
    ...
    fun fromList(list: List<Any?>): UwbDevice {
      ...
      val deviceType: DeviceType? = (list[3] as Int?)?.let {
        DeviceType.ofRaw(it)
      }
...
data class UwbConfig (
  ...
  companion object {
    @Suppress("UNCHECKED_CAST")
    fun fromList(list: List<Any?>): UwbConfig {
      ...
      return UwbConfig(sessionId, sessionKeyInfo, channel, preambleIndex)
    }
  }
  fun toList(): List<Any?> {
    return listOf<Any?>(
...
interface UwbHostApi {
  ...
  fun startPeerSession(peerToken: ByteArray, config: UwbConfig)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: UwbHostApi?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.uwb.UwbHostApi.startControllerSession", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            ...
            var wrapped: List<Any?>
            try {
              api.startControllerSession(configArg)
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.uwb.UwbHostApi.startAccessorySession", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val configArg = args[0] as UwbConfig
            var wrapped: List<Any?>
            try {
              api.startAccessorySession(configArg)
              wrapped = listOf<Any?>(null)
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.uwb.UwbHostApi.startPeerSession", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val peerTokenArg = args[0] as ByteArray
            val configArg = args[1] as UwbConfig
            var wrapped: List<Any?>
            try {
              api.startPeerSession(peerTokenArg, configArg)
              wrapped = listOf<Any?>(null)
            } catch (exception: Throwable) {
              wrapped = wrapError(exception)
            }
...
class UwbFlutterApi(private val binaryMessenger: BinaryMessenger) {
  ...
  fun onShareableConfigurationData(dataArg: ByteArray, peerIdArg: String, callback: (Result<Unit>) -> Unit)
{
    val channelName = "dev.flutter.pigeon.uwb.UwbFlutterApi.onShareableConfigurationData"
    val channel = BasicMessageChannel<Any?>(binaryMessenger, channelName, codec)
    channel.send(listOf(dataArg, peerIdArg)) {
      if (it is List<*>) {
        if (it.size > 1) {
