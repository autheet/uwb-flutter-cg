...
struct UwbDevice {
  ...
  static func fromList(_ list: [Any?]) -> UwbDevice? {
    ...
    let rssi: Int64? = isNullish(list[5]) ? nil : (list[5] is Int64? ? list[5] as! Int64? : Int64(list[5] as! Int32))
...
protocol UwbHostApi {
  ...
  func discoverDevices(deviceName: String, auHourlyDigest: String, completion: @escaping (Result<Void, Error>) -> Void)
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: UwbHostApi?) {
    ...
    if let api = api {
      discoverDevicesChannel.setMessageHandler { message, reply in
        ...
        let auHourlyDigestArg = args[1] as! String
        api.discoverDevices(deviceName: deviceNameArg, auHourlyDigest: auHourlyDigestArg) { result in
          switch result {
          case .success:
            reply(wrapResult(nil))
          case .failure(let error):
...
protocol UwbFlutterApiProtocol {
  ...
  func onUwbRangingResult(data dataArg: UwbData, completion: @escaping (Result<Void, FlutterError>) -> Void)
  func onUwbRangingError(error errorArg: String, completion: @escaping (Result<Void, FlutterError>) -> Void)
...
class UwbFlutterApi: UwbFlutterApiProtocol {
  ...
  func onUwbRangingResult(data dataArg: UwbData, completion: @escaping (Result<Void, FlutterError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.uwb.UwbFlutterApi.onUwbRangingResult"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
  ...
  func onUwbRangingError(error errorArg: String, completion: @escaping (Result<Void, FlutterError>) -> Void) {
    let channelName: String = "dev.flutter.pigeon.uwb.UwbFlutterApi.onUwbRangingError"
    let channel = FlutterBasicMessageChannel(name: channelName, binaryMessenger: binaryMessenger, codec: codec)
