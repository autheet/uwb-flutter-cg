...
class UwbHostApiSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: UwbHostApi?) {
    ...
    let stopUwbSessionChannel = FlutterBasicMessageChannel(name: "dev.flutter.pigeon.uwb.UwbHostApi.stopUwbSession", binaryMessenger: binaryMessenger, codec: codec)
    if let api = api {
      stopUwbSessionChannel.setMessageHandler { message, reply in
        let args = message as! [Any?]
        let deviceArg = args[0] as! UwbDevice
      ...
      stopUwbSessionChannel.setMessageHandler(nil)
    }
